...
public class ConfigureBlock {
    ...
    /**
     * For an unsupported node, declare a new entry in the configure block, and build the configure DSL For a supported
     * node, parse its child nodes
     * 
     * @param node
     */
    ...
    /**
     * For a supported node, iterate over all child nodes, and parse that node to determine if it should be into the
     * configure block, i.e. if the child node is an unsupported node
     * 
    ...
    /**
     * For all parent nodes except the root node, wrap the node name in quotes, and split with a forward slash. For root
     * nodes, neglect to wrap the node name in quotes, and remove hyphens from the tag name
     * 
     * @param parentNames
     */
    ...
    /**
     * If a node's immediate parent is the root node, prepend ".delegate" to the node declaration. Failure to do this
     * can result in DSL errors regarding operations on null objects
     * 
    ...
    /**
     * Strip the trailing forward slash from the string buffer, and append chevrons to end the node declaration
     */
    ...
    /**
     * For a given node, build up a list of parent names, then remove the root parent node, and then reverse the list to
     * given an order of parent node names that matches the order expected by the configure block
     * 
     * @param node
     * @return
     */
    ...
    /**
     * Either create a node with simple content, e.g. <mynode>value</mynode> or create a node with complex content, e.g.
     * <node><childNode>value</childNode></node>
     * 
     * @param node
     */
    ...
    /**
     * Nodes with complex content are declared with mynode('value') but then contain a block of the form {
     * moreContent('myvalue') }
     * 
     * @param node
     * @param nodeList
    ...
    private static void createNodeWithSimpleContent(final Node node) {
        ...
        if (node.hasAttributes()) {
            createAttributes(node); // TODO THIS MAY NOT BE NEEDED, SINCE ATTRIBUTES MAY NOT BE CONSIDERED AS CHILDREN,
                                    // SO THIS WOULD NEVER BE HIT
        } else {
    ...
    /**
     * Create the content of a method declaration. Numeric and boolean values are written as-is, string values are
     * wrapped in quotes and escaped
     * 
     * @param node
     */
    ...
    /**
     * Create a node that only contains attributes, no value, e.g. <mynode plugin="1.2.3"/>
     * 
     * @param node
     */
    ...
    private static void createAttributes(final Node node) {
        ...
        for (int i = 0; i < nmap.getLength(); i++) {
            ...
            configureBlockStringBuffer.append(mappedNode.getNodeName()).append(":").append("'")
                    .append(mappedNode.getNodeValue()).append("'").append(",");
        }
