...
public class ProjectConvertor {
    ...
    /**
     * Perform an authenticated request to the target Jenkins, and pull back the config.xml for a specific job. Parse
     * the config.xml using JAXB to construct supported JobDSL calls. Next, generate the configure block using a
     * DocumentFactory, and then write the complete DSL to a file.
     * 
     ...
     */
    public static void convert(final String jenkinsServer, final String username, final String apiToken,
            final String jenkinsPort, final String projectName, final String jobName)
